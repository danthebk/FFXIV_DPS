...
dps_app.dps_controller = function ($scope) {
    ...
    $scope.stats_calculate_init = function () {
        ...
        //rates
        $scope.stats_main.criticalRate = $scope.fCHR($scope.stats_main.criticalDelta); //((Math.floor(200 * $scope.stats_main.criticalDelta / $scope.base_stats.levelMod) + 50) / 1000) * 100;
        $scope.stats_main.directhitRate = $scope.fDHR($scope.stats_main.directhitDelta); //((Math.floor(550 * $scope.stats_main.directhitDelta / $scope.base_stats.levelMod)) / 1000) * 100;

        //damage multiplier
        $scope.stats_main.criticalDamage = $scope.fCSTR($scope.stats_main.criticalDelta); //((Math.floor(200 * $scope.stats_main.criticalDelta / $scope.base_stats.levelMod) + 1400) / 1000) * 100;
